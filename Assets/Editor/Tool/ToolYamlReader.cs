...
//		ad.AnimationClip.m_ScaleCurves.Clear();
	...
	static Vector3CurveData.Vector3 CompressVector3(Vector3CurveData.Vector3 v, int num = 3)
	{
		...
		v.x = double.Parse(v.x.ToString("f3"));
		v.y = double.Parse(v.y.ToString("f3"));
		v.z = double.Parse(v.z.ToString("f3"));
		return v;
	}
	...
	static QuaternionCurveData.Quaternion CompressQuaternion(QuaternionCurveData.Quaternion q, int num = 3)
	{
		...
		q.x = double.Parse(q.x.ToString("f3"));
     	q.y = double.Parse(q.y.ToString("f3"));
		q.z = double.Parse(q.z.ToString("f3"));
		q.w = double.Parse(q.w.ToString("f3"));
		return q;
    }
}
